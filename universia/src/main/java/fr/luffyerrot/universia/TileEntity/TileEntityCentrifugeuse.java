...
import fr.luffyerrot.universia.Recipes.CentrifugeuseRecipes;
...
public class TileEntityCentrifugeuse extends TileEntityLockable implements ITickable {
    ...
    public String getName() {
        return hasCustomName() ? this.customName : "Centrifugeuse";
    }
    ...
    public ItemStack getRecipeResult() {
        return CentrifugeuseRecipes.getRecipeResult(new ItemStack[] {this.getStackInSlot(1), this.getStackInSlot(2), this.getStackInSlot(3), this.getStackInSlot(4), this.getStackInSlot(5), this.getStackInSlot(6)});
    }
    ...
    public boolean canSmelt() {
        ...
        if (result != null) {
            ...
            ItemStack slot8 = this.getStackInSlot(8);
            ItemStack slot9 = this.getStackInSlot(9);
            ItemStack slot10 = this.getStackInSlot(10);
            ItemStack slot11 = this.getStackInSlot(11);

            if (slot8.isEmpty() && slot9.isEmpty() && slot10.isEmpty() && slot11.isEmpty())
                return true;

            int newStackSize1 = slot8.getCount() + result.getCount();
            int newStackSize2 = slot9.getCount() + result.getCount();
            int newStackSize3 = slot10.getCount() + result.getCount();
            int newStackSize4 = slot11.getCount() + result.getCount();
            if (newStackSize1 <= this.getInventoryStackLimit() && newStackSize2 <= this.getInventoryStackLimit() && 
            		newStackSize3 <= this.getInventoryStackLimit() && newStackSize4 <= this.getInventoryStackLimit()) {
                return true;
            }
        }
        return false;
    ...
    public void smelt() {
        ItemStack result = this.getRecipeResult();
        ItemStack result2 = new ItemStack(Items.BUCKET);
        ...
        ItemStack bucket = this.getStackInSlot(7);
        ItemStack end1 = this.getStackInSlot(8);
        ItemStack end2 = this.getStackInSlot(9);
        ItemStack end3 = this.getStackInSlot(10);
        ItemStack end4 = this.getStackInSlot(11);
        
        if (bucket.isEmpty()){
            this.setInventorySlotContents(7, result2.copy());
        }else {
        	bucket.grow(result2.getCount());
        }
        
        if (end1.isEmpty()) {
            this.setInventorySlotContents(8, result.copy());
        } else {
        	end1.grow(result.getCount());
        }
        if (end2.isEmpty()) {
            this.setInventorySlotContents(9, result.copy());
        } else {
        	end2.grow(result.getCount());
        }
        if (end3.isEmpty()) {
            this.setInventorySlotContents(10, result.copy());
        } else {
        	end3.grow(result.getCount());
        }
        if (end4.isEmpty()) {
            this.setInventorySlotContents(11, result.copy());
        } else {
        	end4.grow(result.getCount());
        }
    }
