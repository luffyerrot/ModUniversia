...
public class BlockCropCoca extends BlockCrops {
	
    public BlockCropCoca(){
        setUnlocalizedName("coca_crop");
        setRegistryName("coca_crop");
    }

    @Override
    protected Item getSeed(){

        return ModItems.coca_seed;
    }

    protected Item getCropLeave(){

        return ModItems.coca_leave;
    }
    
    protected Item getCropBay(){

        return ModItems.coca_bay;
    }
    ...
    public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
        ...
        if (age >= getMaxAge())
        {
            ...
            drops.add(new ItemStack(this.getCropLeave(), 1, 0));
            drops.add(new ItemStack(this.getCropBay(), 2, 0));
        }
    }
        ...
        if (!worldIn.isRemote){
            if (this.isMaxAge(state)){
            	if (playerIn.getHeldItemMainhand().getItem() == ModItems.wood_faucille || playerIn.getHeldItemMainhand().getItem() == ModItems.stone_faucille ||
	        		...
	        		worldIn.spawnEntity(new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ModItems.coca_leave, 1)));
	        		worldIn.spawnEntity(new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ModItems.coca_bay, 2)));
